...
public class KeycloakSmsAuthenticator implements Authenticator {
  ...
  public void authenticate(AuthenticationFlowContext context) {
    ...
    List<String> mobileNumberCreds =
        user.getAttribute(KeycloakSmsAuthenticatorConstants.ATTR_MOBILE);
  ...
  private void sendEmailViaSunbird(Map<String, Object> otpResponse, String userEmail) {
    if (null != otpResponse) {
      ...
      HttpClient.post(request,
          (System.getenv(Constants.SUNBIRD_WEB_URL) + Constants.SEND_NOTIFICATION_URI),
  ...
  private Map<String, Object> sendSMS(AuthenticationFlowContext context, String mobileNumber) {
    ...
    if (KeycloakSmsAuthenticatorUtil.sendSmsCode(mobileNumber, code,
      ...
      Response challenge =
          context.form().setError("SMS could not be sent.").createForm("sms-validation-error.ftl");
  ...
  private void sendEmail(AuthenticationFlowContext context) {
    ...
    // we don't want people guessing usernames, so if there was a problem obtaining the user, the
    // user will be null.
    ...
    if (actionTokenUserId != null && Objects.equals(user.getId(), actionTokenUserId)) {
      logger.debugf(
          "Forget-password triggered when reauthenticating user after authentication via action token. Skipping "
              + CREDENTIAL_TYPE + " screen and using user '%s' ",
          user.getUsername());
      context.success();
      return;
    }
    ...
    ResetCredentialsActionToken token = new ResetCredentialsActionToken(user.getId(),
        absoluteExpirationInSecs, authenticationSession.getId());
    ...
    try {
      ...
      context.getSession().getProvider(EmailTemplateProvider.class).setRealm(context.getRealm())
          .setUser(user).sendPasswordReset(link, expirationInMinutes);

      event.clone().event(EventType.SEND_RESET_PASSWORD).user(user)
          .detail(Details.USERNAME, username).detail(Details.EMAIL, user.getEmail())
          .detail(Details.CODE_ID, authenticationSession.getId()).success();
      context.forkWithSuccessMessage(new FormMessage(Messages.EMAIL_SENT));


      Response challenge = context.form().createForm("password-reset-email.ftl");
      context.failureChallenge(AuthenticationFlowError.UNKNOWN_USER, challenge);

    } catch (EmailException e) {
      event.clone().event(EventType.SEND_RESET_PASSWORD).detail(Details.USERNAME, username)
          .user(user).error(Errors.EMAIL_SEND_FAILED);
  ...
  public void action(AuthenticationFlowContext context) {
    ...
    logger.debug(
        "KeycloakSmsAuthenticator@action called ... for User = " + context.getUser().getUsername());
    CODE_STATUS status = validateCode(context);
    Response challenge = null;
    switch (status) {
      case EXPIRED:
        logger.debug("KeycloakSmsAuthenticator@action - EXPIRED");
        challenge = context.form().setError("code is expired").createForm("sms-validation.ftl");
      ...
      case INVALID:
        ...
        if (context.getExecution()
            .getRequirement() == AuthenticationExecutionModel.Requirement.OPTIONAL
          ...
          challenge = context.form().setError("Invalid code specified, please enter it again")
          ...
          logger.debug(
              "KeycloakSmsAuthenticator@action - " + context.getExecution().getRequirement());
          logger.debug(
              "KeycloakSmsAuthenticator@action - " + context.getExecution().getAuthenticator());
          logger.debug("KeycloakSmsAuthenticator@action - "
              + context.getExecution().getAuthenticatorConfig());
          ...
          logger
              .debug("KeycloakSmsAuthenticator@action - " + context.getExecution().getParentFlow());
  ...
  // When the code is validated on another node (in a clustered environment) the other nodes have
  // access to it's values too.
  private void storeSMSCode(AuthenticationFlowContext context, String code, Long expiringAt) {
    logger.debug(
        "KeycloakSmsAuthenticator@storeSMSCode" + "User name = " + context.getUser().getUsername());
  ...
  protected CODE_STATUS validateCode(AuthenticationFlowContext context) {
    ...
    logger.debug("KeycloakSmsAuthenticator@validateCode " + "User name = "
        + context.getUser().getUsername());
    logger.debug("KeycloakSmsAuthenticator@validateCode " + "Expected code = "
        + expectedCode.getValue() + "    entered code = " + enteredCode);

    if (expectedCode != null) {
      result =
          enteredCode.equals(expectedCode.getValue()) ? CODE_STATUS.VALID : CODE_STATUS.INVALID;
    }
  ...
  public boolean configuredFor(KeycloakSession session, RealmModel realm, UserModel user) {
    logger.debug("KeycloakSmsAuthenticator@validateCode configuredFor called ... session=" + session
  ...
  public void setRequiredActions(KeycloakSession session, RealmModel realm, UserModel user) {
    logger.debug("KeycloakSmsAuthenticator@validateCode - setRequiredActions called ... session="
