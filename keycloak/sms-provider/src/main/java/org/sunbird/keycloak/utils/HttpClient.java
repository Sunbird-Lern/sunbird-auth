...
public class HttpClient {
  ...
  public static HttpResponse post(Map<String,Object> requestBody,String uri, String authorizationKey){
    ...
    try {
      ...
      logger.debug("HttpClient@post: response.getStatusLine().getStatusCode() :"+response.getStatusLine().getStatusCode());
      return response;
    } catch (Exception e) {
      ...
      if(null != client)
        try {
          ...
          logger.error("HttpClient:post : Exception occurred while closing client:" + e);
        }  
    }
    return null;
